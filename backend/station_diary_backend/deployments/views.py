...
class CreateDeploymentView(APIView):
    ...
    def put(self, request, format=None):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "Deployment have been successfully created"}, status=status.HTTP_201_CREATED)
...
class EditDeploymentView(APIView):
    ...
    def post(self, request, deployment_id, format=None):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "Deployment have been successfully updated"}, status=status.HTTP_200_OK)
